...
消息中间件（Message Middleware）是系统之间传递消息的基础设施，也常被称为消息队列（Message Queue，MQ）。

简单理解：消息中间件就像一个中转站。生产者把消息发送到中间件后即可继续处理其他任务；中间件负责暂存和转发消息；消费者按照自己的处理能力读取消息。生产者和消费者不需要直接调用，也不必同时在线。

```text
生产者 -> 消息中间件 -> 消费者
下单服务 -> 订单创建消息 -> 库存、积分、短信服务
```

消息中间件主要解决四类问题：

- 异步：非核心操作不必阻塞主流程。例如下单成功后，将发送短信改为异步处理，缩短接口响应时间。
- 解耦：生产者只负责发送消息，不需要知道由哪些系统处理。新增积分服务时，通常不需要修改下单服务。
- 削峰：流量高峰时先把请求写入队列，消费者按照自身能力处理，避免瞬时流量压垮数据库或下游服务。
- 可靠传输：通过持久化、副本、确认和重试等机制，降低消息因服务故障而丢失的风险。

引入消息中间件也会增加系统复杂度，需要重点处理消息丢失、重复消费、消息顺序、消费积压、数据一致性和中间件高可用等问题。

面试时可以这样回答：

> 消息中间件是生产者和消费者之间的消息传输组件。生产者发送消息后不必等待消费者立即处理，消费者可以按自己的速度消费。它的核心价值是异步、解耦、削峰和可靠传输，但同时需要解决消息丢失、重复消费、顺序性和数据一致性等问题。
...
阅读后续内容前，先了解 Kafka 副本和位点的常用名称：

| 名称 | 全称 | 含义 |
|---|---|---|
| AR | Assigned Replicas | 分区的全部已分配副本集合，包含 Leader、ISR 和 OSR。 |
| ISR | In-Sync Replicas | 与 Leader 保持同步的副本集合，Leader 发生故障时会优先从中选举新 Leader。 |
| OSR | Out-of-Sync Replicas | 未能及时跟上 Leader 的副本集合，不属于 ISR。 |
| Leader | Leader Replica | 分区的主副本，生产者写入和消费者读取都由它处理。 |
| Follower | Follower Replica | 分区的从副本，从 Leader 拉取消息以保持同步。 |
| LEO | Log End Offset | 日志末端位点，表示下一条消息将写入的位置。 |
| HW | High Watermark | 高水位线，消费者通常只能读取 HW 之前、已完成副本同步的消息。 |

关系可以简单理解为：`AR = ISR + OSR`。生产环境通常结合 `acks=all` 和 `min.insync.replicas`，要求 ISR 中有足够副本确认消息后再返回成功。
...
Kafka 快主要因为顺序写磁盘、Page Cache、零拷贝、批量处理、分区并行和消息压缩。

展开：

1. 顺序写磁盘

   Kafka 将消息追加到分区日志末尾，避免频繁修改文件中间的数据。顺序写减少了磁盘寻址开销，性能远高于随机写。

2. Page Cache

   Page Cache 是操作系统对磁盘文件的内存缓存。Kafka 写消息时，数据通常先进入 Page Cache，再由操作系统批量刷入磁盘；消费者读取刚写入的热点消息时，也通常可以直接从 Page Cache 获取，不必再次访问磁盘。

   ```text
   Producer -> Kafka -> Page Cache -> 磁盘
                              |
                              +-> Consumer
   ```

   Kafka 不需要在 JVM 堆中维护一份大型消息缓存，因此还能减少堆内存占用和 GC 压力。写入 Page Cache 不等于已经物理落盘，Kafka 的可靠性还需要依靠副本、ISR、`acks=all` 和 `min.insync.replicas`。

3. 零拷贝

   Kafka 发送日志文件时可以利用 `sendfile` 等零拷贝能力，减少数据在内核空间和用户空间之间的复制，从而降低 CPU 消耗和上下文切换次数。

4. 批量处理

   Producer 会把多条消息组成一个批次再发送，Broker 和 Consumer 也会按批处理数据，从而减少网络请求、磁盘写入和系统调用次数。

5. 分区并行

   一个 Topic 可以拆成多个 Partition，并分布到不同 Broker。不同分区可以并行生产和消费，从而利用多台服务器的 CPU、磁盘和网络资源进行水平扩展。

6. 消息压缩

   Producer 可以按批次使用 `lz4`、`snappy` 或 `zstd` 压缩消息。压缩会消耗少量 CPU，但能减少网络传输量、磁盘占用和磁盘 IO。

面试总结：

> Kafka 通过追加日志实现顺序写，利用操作系统 Page Cache 加速文件读写并减少 JVM GC，通过零拷贝降低数据复制开销，再结合批量处理、消息压缩和多分区并行，实现高吞吐和水平扩展。
